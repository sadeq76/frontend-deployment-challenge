...
describe('http client', () => {
  ...
  it('serializes JSON request bodies and reports typed HTTP failures', async () => {
    const fetcher = vi.fn().mockResolvedValue(new Response(JSON.stringify({ message: 'nope' }), { status: 422, headers: { 'content-type': 'application/json' } }))
    const client = createHttpClient({ baseUrl: 'https://example.test', fetch: fetcher })
    await expect(client.request('/products', { method: 'POST', body: { title: 'A' } })).rejects.toMatchObject({ kind: 'http', response: { status: 422, data: { message: 'nope' } } })
    expect(fetcher.mock.calls[0][1].body).toBe('{"title":"A"}')
  })

  it('returns null for an empty successful response', async () => {
    const client = createHttpClient({ baseUrl: 'https://example.test', fetch: vi.fn().mockResolvedValue(new Response(null, { status: 204 })) })
