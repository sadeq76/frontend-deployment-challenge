...
export function useProductFilters(products: Ref<Product[]>) {
  ...
  const sort = computed<ProductSort>(() => {
    ...
    return value === 'price-asc' || value === 'price-desc' || value === 'rating-desc' || value === 'title-asc' ? value : 'default'
  })
  const categories = computed(() => [...new Set(products.value.map((product: Product) => product.category))].sort())
  const filteredProducts = computed(() => {
    const normalizedQuery = query.value.trim().toLocaleLowerCase()
    const list = products.value.filter((product: Product) => (!normalizedQuery || product.title.toLocaleLowerCase().includes(normalizedQuery)) && (!category.value || product.category === category.value))
  ...
  const update = async (changes: Partial<{ q: string; category: string; sort: ProductSort }>) => {
    ...
    await router.push({ query: Object.fromEntries(Object.entries(next).filter(([key, value]) => value && !(key === 'sort' && value === 'default'))) })
  }
...
export function useProductFormat() {
  ...
  const price = (value: number) => new Intl.NumberFormat(locale.value === 'fa' ? 'fa-IR' : 'en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 2 }).format(value)
  const number = (value: number) => new Intl.NumberFormat(locale.value === 'fa' ? 'fa-IR' : 'en-US').format(value)
  return { price, number }
}
