...
function combinedSignal(external: AbortSignal | undefined, timeoutMs: number | undefined) {
  ...
  return {
    ...
    cleanup: () => { clearTimeout(timer); external?.removeEventListener('abort', abort) },
...
export function createHttpClient(options: HttpClientOptions) {
  ...
  async function request<TResponse, TBody = unknown>(path: string, config: RequestConfig<TBody> = {}): Promise<HttpResponse<TResponse>> {
    ...
    if (hasBody) {
      if (config.body instanceof FormData || config.body instanceof URLSearchParams || typeof config.body === 'string' || config.body instanceof Blob) body = config.body
      else { headers.set('content-type', headers.get('content-type') ?? 'application/json'); body = JSON.stringify(config.body) }
    }
    const control = combinedSignal(config.signal, config.timeoutMs ?? options.timeoutMs)
    try {
      const response = await fetcher(buildUrl(options.baseUrl, path, config.query), { method, headers, body, signal: control.signal })
      const result: HttpResponse<TResponse> = { data: await parseResponse(response) as TResponse | null, status: response.status, statusText: response.statusText, headers: response.headers }
  ...
  return { request, get: <T>(path: string, config?: Omit<RequestConfig<never>, 'method'>) => request<T>(path, { ...config, method: 'GET' }) }
}
